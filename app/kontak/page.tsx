...
async function getKontak(): Promise<{
  ...
  try {
    ...
    const [settingsResult, socialResult] = await Promise.all([
      supabase.from("site_settings").select("address, email, phone").eq("id", 1).maybeSingle(),
      supabase
        .from("social_links")
    ...
    if (settingsResult.error || socialResult.error) {
      ...
      return { data: null, error: "Informasi kontak sementara tidak dapat ditampilkan." };
    }
    ...
    return {
      data: {
        contact: {
          ...
          // Handle Instagram belum disimpan di database (url social_links null).
          instagram: null,
        },
        ...
        <Container>
          ...
          <div className="mt-8">
            {error ? (
              ...
              <ContactInfoCards values={data?.contact ?? { address: "", email: "", phone: "", instagram: null }} />
            )}
          </div>
        </Container>
      </Section>
          ...
          <div className="grid gap-8 lg:grid-cols-5">
            ...
            <div className="lg:col-span-2">
              <div className="rounded-card border border-border bg-background p-6">
                <h2 className="text-h4 text-foreground">Cara Lain Menghubungi Kami</h2>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                  Jika ingin berbincang lebih lanjut atau berdiskusi langsung,
                  Anda dapat mengunjungi lingkungan RT 04 RW 08 Srengseng Sawah
                  atau menunggu pembaruan kontak resmi berikutnya.
                </p>
                <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
                  Formulir ini bukan sarana komunikasi yang aktif. Seluruh kanal
                  kontak pada halaman ini masih berupa placeholder dan akan
                  diperbarui setelah kontak resmi tersedia.
                </p>
              </div>
            </div>
          </div>
